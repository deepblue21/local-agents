...
const INSTALLED_MODELS_SEED = [
  { id: 'deepseek',     name: 'deepseek-v3:67b',         family: 'DeepSeek',  params: '67B',  quant: 'q4_K_M', size: 38.6, totalLayers: 80, contextWindow: 16384, vramFootprint: 38.6, lastUsed: '2 min ago' },
  { id: 'qwen35-14b',   name: 'qwen3.5:14b',             family: 'Qwen',      params: '14B',  quant: 'q4_K_M', size: 9.84, totalLayers: 64, contextWindow: 8192,  vramFootprint: 9.8,  lastUsed: '34 min ago' },
  { id: 'llama-70b',    name: 'llama3.1:70b-instruct',   family: 'Meta',      params: '70B',  quant: 'q4_K_M', size: 40.2, totalLayers: 80, contextWindow: 8192,  vramFootprint: 40.2, lastUsed: '2 hours ago' },
  { id: 'gemma2-27b',   name: 'gemma2:27b-it',           family: 'Gemma',     params: '27B',  quant: 'q4_K_M', size: 16.0, totalLayers: 46, contextWindow: 8192,  vramFootprint: 16.0, lastUsed: 'Yesterday' },
  { id: 'deepseek-cod', name: 'deepseek-coder:6.7b',     family: 'DeepSeek',  params: '6.7B', quant: 'q5_K_M', size: 4.78, totalLayers: 32, contextWindow: 4096,  vramFootprint: 4.8,  lastUsed: 'Last week' },
];

// Compute per-node assignment given a deployment configuration
function computeAssignment(allNodes, model, strategy, pinnedNodeId, perNode, installedById) {
  const out = {};
  if (strategy === 'shard' && model) {
    const weights = allNodes.map(n => n.unifiedTotal);
    ...
    allNodes.forEach((n, i) => {
      ...
      const role = n.role === 'master' ? 'host' : 'shard';
      out[n.id] = {
        task: target > 0 ? `${role} · layers ${start}–${end} (${model.family} ${model.params})` : 'idle',
        layersHosted: Math.max(0, target),
        modelId: model.id,
  ...
  if (strategy === 'pin' && model) {
    allNodes.forEach(n => {
      if (n.id === pinnedNodeId) {
        out[n.id] = { task: `host · all ${model.totalLayers} layers (${model.family} ${model.params})`, layersHosted: model.totalLayers, modelId: model.id, modelName: model.name };
      } else {
        out[n.id] = { task: 'idle · awaiting dispatch', layersHosted: 0, modelId: null, modelName: null };
      }
    });
    return out;
  }
  if (strategy === 'per-node') {
    allNodes.forEach(n => {
      const mId = perNode[n.id];
      const m = mId ? installedById[mId] : null;
      out[n.id] = m
        ? { task: `host · ${m.name}`, layersHosted: m.totalLayers || 0, modelId: m.id, modelName: m.name }
        : { task: 'idle · no model assigned', layersHosted: 0, modelId: null, modelName: null };
    });
    return out;
  }
  allNodes.forEach(n => { out[n.id] = { task: n.task, layersHosted: n.layersHosted, modelId: model?.id || null, modelName: model?.name || null }; });
  return out;
}

// ── Real backend hook takes over when api.jsx is loaded ──────────────────
function useCluster() {
  if (typeof window !== 'undefined' && window.useClusterFromAPI) {
    return window.useClusterFromAPI();
  }
  return useClusterMock();
}

function useClusterMock() {
  const [baseNodes, setBaseNodes] = useState(NODES_SEED);
  const [installedModels, setInstalledModels] = useState(INSTALLED_MODELS_SEED);
  const [activeModelId, setActiveModelId] = useState('deepseek');
  const [strategy, setStrategy] = useState('shard'); // 'shard' | 'pin' | 'per-node'
  const [pinnedNodeId, setPinnedNodeId] = useState(NODES_SEED[1].id);
  const [perNode, setPerNode] = useState({
    [NODES_SEED[0].id]: 'qwen35-14b',
    [NODES_SEED[1].id]: 'deepseek',
    [NODES_SEED[2].id]: 'gemma2-27b',
  });

  useEffect(() => {
    const id = setInterval(() => {
      setBaseNodes(ns => ns.map(n => {
  ...
  const installedById = useMemo(() => Object.fromEntries(installedModels.map(m => [m.id, m])), [installedModels]);
  const model = installedById[activeModelId] || ACTIVE_MODEL;

  // Enriched nodes: base hardware metrics + derived deployment fields
  const nodes = useMemo(() => {
    const assign = computeAssignment(baseNodes, model, strategy, pinnedNodeId, perNode, installedById);
    return baseNodes.map(n => {
      const a = assign[n.id] || {};
      const idle = (a.layersHosted || 0) === 0;
      return {
        ...n,
        task: a.task ?? n.task,
        layersHosted: a.layersHosted ?? n.layersHosted,
        runningModelId: a.modelId,
        runningModelName: a.modelName,
        status: idle ? 'idle' : (n.status === 'idle' ? 'online' : n.status),
        throughput: idle ? 0 : n.throughput,
      };
    });
  }, [baseNodes, model, strategy, pinnedNodeId, perNode, installedById]);
  ...
  // Deployment mutators
  const deployment = { activeModelId, strategy, pinnedNodeId, perNode };
  const applyDeployment = (cfg) => {
    if (cfg.activeModelId !== undefined) setActiveModelId(cfg.activeModelId);
    if (cfg.strategy !== undefined)      setStrategy(cfg.strategy);
    if (cfg.pinnedNodeId !== undefined)  setPinnedNodeId(cfg.pinnedNodeId);
    if (cfg.perNode !== undefined)       setPerNode(cfg.perNode);
  };

  const addInstalled = (m) => setInstalledModels(ms => ms.find(x => x.id === m.id) ? ms : [m, ...ms]);
  const removeInstalled = (id) => {
    setInstalledModels(ms => ms.filter(m => m.id !== id));
    if (activeModelId === id) {
      const remaining = installedModels.find(m => m.id !== id);
      if (remaining) setActiveModelId(remaining.id);
    }
  };

  return {
    nodes, totals, model,
    installedModels, installedById,
    deployment, applyDeployment,
    addInstalled, removeInstalled,
  };
}
...
Object.assign(window, {
  ...
  useCluster, ACTIVE_MODEL, NODES_SEED, INSTALLED_MODELS_SEED,
